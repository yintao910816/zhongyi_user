...
class dialogViewModel: NSObject {
    ...
    var model : dialogModel? {
        didSet{
            ...
            if model?.status == "1"{ //自己的消息
                let headX = SCREEN_WIDTH - cellPadding - headWidth
                if model?.type == TypeText {
                    let tempWordSize = HCGetSize(content: model?.content as! NSString, maxWidth: wordMaxW, font: UIFont.systemFont(ofSize: 14))
                    let tempTimeSize = HCGetSize(content: model?.consultTime as! NSString, maxWidth: wordMaxW, font: UIFont.systemFont(ofSize: 14))
                    ...
                    headIVF = CGRect.init(x: headX, y: cellMargin + bubbleSize.height - headWidth, width: headWidth, height: headWidth)
                    bubbleF = CGRect.init(x: headX - headToBubble - bubbleSize.width, y: cellMargin, width: bubbleSize.width, height: bubbleSize.height)
                    wordsF = CGRect.init(x: headX - headToBubble - bubbleSize.width + bubblePadding, y: cellMargin + bubblePadding, width: tempWordSize.width + 2, height: tempWordSize.height)
                ...
                if model?.type == TypeText{
                    ...
                    headIVF = CGRect.init(x: cellPadding, y: cellMargin + bubbleSize.height - headWidth, width: headWidth, height: headWidth)
                    bubbleF = CGRect.init(x: cellPadding + headWidth + headToBubble, y: cellMargin, width: bubbleSize.width, height: bubbleSize.height)
                    wordsF = CGRect.init(x: cellPadding + headWidth + headToBubble + bubblePadding, y: cellMargin + bubblePadding, width: tempWordSize.width + 2, height: tempWordSize.height)
